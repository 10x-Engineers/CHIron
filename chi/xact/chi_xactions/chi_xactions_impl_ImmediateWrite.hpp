        ...
        : Xaction<config>(XactionType::ImmediateWrite, firstFlit, retried)
    {
        ...
        ) [[unlikely]]
        {
            this->firstDenial = this->RequestFlitDenied(XactDenial::DENIED_REQ_OPCODE, this->first,
                "This Opcode is not type of / supported by Immediate Write transaction");
            return;
        }
    ...
    inline bool XactionImmediateWrite<config>::IsDataComplete(const Global<config>& glbl) const noexcept
    {
        ...
        for (auto iter = this->subsequence.begin(); iter != this->subsequence.end(); iter++, index++)
        {
            ...
            )
            {
                ...
                // transferred must be sent" -- a cancelled >32B Ptl write (e.g. a 64B
                // WriteUniquePtl) sends one WriteDataCancel per data packet (Sec 2.10.4
                // / Table 2-15). The original `return true;` retired the transaction
                // after the first cancel beat, so the mandatory subsequent beat(s) were
                // denied DENIED_DAT_TXNID_NOT_EXIST (rc=59). Accumulate DataIDs exactly
                // like NonCopyBackWrData so completion requires the full size mask.
                collectedDataID |= details::CollectDataID<config>(
                    this->first.flit.req.Size(), iter->flit.dat.DataID());
        }
